    ...
    public class TramitesController : Controller
    {
        ...
        public ActionResult Index(string currentFilter, DateTime strt, DateTime end, string searchString, int? page, int lista)
        {
            ...
            if (lista == 0)
            {
                ...
                var tramites =
                    ...
                    where tr.Email == email && tr.HoraInicio >= stParse && tr.HoraFim <= enParse
            ...
            else
            {
                ...
                var tramites =
                    ...
                    orderby tr.TramiteID descending
                    join al in _context.TramiteDePara on tr.TramiteID equals al.IdTramite into alGrupo
                    where tr.Email == email && tr.HoraInicio >= strt && tr.HoraFim <= end
                    select new GrupoTramite()
                    {
                        Tramite = tr,
                        DePara = (from alte in alGrupo
        ...
        public async Task<IActionResult> Create([Bind("TramiteID,OrigemTramite,Time,Atividade,Retrabalho,Email,Projeto,Pbi,Tarefa,Titulo,HoraInicio,HoraFim,Obs,TramiteEditado,Sprint")] Tramite tramite)
        {
            if (ModelState.IsValid)
            {
                ...
                foreach (var group in _context.Tramite.Where(x => x.Tarefa == tramite.Tarefa).GroupBy(x => x.Tarefa))
                {
                    ...
                    var jsonTempoTarefa = "[{\"op\":\"add\",\"path\":\"/fields/Custom.LinxEsforcoReal\",\"value\":\"" + tempoTarefaComPonto + "\"},{\"op\":\"add\",\"path\":\"/fields/System.History\",\"value\":\"Tramite criado manualmente, esforço real atualizado para " + tempoTarefaComVirgula + "h.\"}]";
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    foreach (var group in _context.Tramite.Where(x => x.Tarefa == tramite.Tarefa).GroupBy(x => x.Tarefa))
                    {
                        ...
                        var jsonTempoTarefa = "[{\"op\":\"add\",\"path\":\"/fields/Custom.LinxEsforcoReal\",\"value\":\"" + tempoTarefaComPonto + "\"},{\"op\":\"add\",\"path\":\"/fields/System.History\",\"value\":\"Tramite editado, esforço real atualizado para " + tempoTarefaComVirgula + "h.\"}]";
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            foreach (var group in _context.Tramite.Where(x => x.Tarefa == tramite.Tarefa).GroupBy(x => x.Tarefa))
            {
                ...
                var jsonTempoTarefa = "[{\"op\":\"add\",\"path\":\"/fields/Custom.LinxEsforcoReal\",\"value\":\"" + tempoTarefaComPonto + "\"},{\"op\":\"add\",\"path\":\"/fields/System.History\",\"value\":\"Tramite deletado, esforço real atualizado para " + tempoTarefaComVirgula + "h.\"}]";
